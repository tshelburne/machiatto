import type {Label, Context, Runnable} from './types.d.ts';
...
export class Suite {
    ...
    static clone(suite: Suite, label?: Label, ctx?: Context): Suite {
        ...
        const nextCtx = ctx ?? Object.entries(suite.ctx).reduce((o, [k, v]) => ({...o, [k]: v}), {})
    ...
    async run() {
        ...
        await Promise.all(this.tests.map(async (t) => {
            ...
            Deno.test(t.label, t.run.bind(this.ctx))
...
function runSeries(ps: Runnable[], ctx: Context = {}): Promise<void> {
    return ps.reduce<Promise<void>>((a, p) => a.then(p.bind(ctx)), voidP)
}
